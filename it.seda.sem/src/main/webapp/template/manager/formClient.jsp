...
<title>${x:i18n('formClient.page.Title')}</title>
	...
	<c:if test="${action=='delete'}">
	  <c:url value="/manager/client/${clientData.id}" var="actionForm">
		<c:param name="pageNumber" value="${pageNumber}"/>
		<c:param name="rowsPerPage" value="${rowsPerPage}"/>
	</c:url>
	</c:if> 
	...
	<div class="newClient" style="margin-left:10px;min-width:70px;max-width:200px;">
	  <a href="<c:url value="/manager/client"/>"  style="margin-right:10px;"><input type="submit" value="${x:i18n('client.manager.newClient')}" style="width:100%;"/></a>
	 </div>
