...
<title>${x:i18n('formServer.page.Title')}</title>
	...
	<div class="newServer" style="margin-left:10px;min-width:70px;max-width:200px;">
	  <a href="<c:url value="/manager/server"/>" style="margin-right:10px;"><input type="submit" value="${x:i18n('server.manager.newServer')}" style="width:100%;"/></a>
	</div> 
	  
	 <form:form method="${method}" action="${actionForm}" commandName="serverData">
	 ...
	 <div class="seda-ui-divrow">
	  ...
	  <form:input readonly="${readOnly}" path="ip"  class="seda-ui-inputrow" style="width:200px" />
	  <form:errors path="ip"  cssStyle="color:red;float:left;margin-left: 10px;"/>
	 </div>
	...
	<x:dgcolumn title="${x:i18n('server.manager.nome')}">${serverRow.ip}</x:dgcolumn>
	  ...
	  <x:dgcolumn title="edit"><a href="<c:url value="/manager/server/${serverRow.id}?action=edit&pageNumber=${pageNumber}&rowsPerPage=${rowsPerPage}"/>">Edit</a></x:dgcolumn>
	  <x:dgcolumn title="cancel"><a href="<c:url value="/manager/server/${serverRow.id}?action=delete&pageNumber=${pageNumber}&rowsPerPage=${rowsPerPage}"/>">Cancel</a></x:dgcolumn>
	</x:datagrid>
