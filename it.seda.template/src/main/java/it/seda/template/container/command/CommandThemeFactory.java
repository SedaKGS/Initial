...
public class CommandThemeFactory {
	 ...
	 */
	public static CommandTheme parseTheme(String defaultTheme, String themeArgs) {
		if(includePattern(themeArgs)){
			TreeSet<String> treeThemes=resolve(defaultTheme, themeArgs);
			return new IncludeTheme(defaultTheme, treeThemes);
		}
		if(excludePattern(themeArgs)){
			TreeSet<String> treeThemes=resolve(defaultTheme, themeArgs);
			return new ExcludeTheme(defaultTheme, treeThemes);
		}
		return new DummyTheme(defaultTheme, new TreeSet<String>());	
	}
	...
	public static TreeSet<String> resolve(String defaultTheme, String themeArgs) {
		 String themes=null;
		 ...
		 TreeSet<String> outTree=resolveThemes(defaultTheme, themes); 
		 return outTree;
	}
	...
	public static TreeSet<String> resolveThemes(String defaultTheme, String themes) {
		
		String [] arrayThemes=null;
		TreeSet<String> themesSet=new TreeSet<String>(new ThemeComparator<String>());
        if(themes==null){
        	themesSet.add(defaultTheme);
        	return themesSet;
		}
		if(themes.contains(",")){
			themes.split(",");
			...
			arrayThemes=new String[]{themes.isEmpty()?defaultTheme:themes};
		}
	...
	static public boolean includePattern(String themeArgs){
		...
		if(themeArgs.startsWith(CommandTheme.INCLUDE)
				&&themeArgs.endsWith(CommandTheme.END)) {
			return true;
		}
		if(!themeArgs.startsWith(CommandTheme.INCLUDE)
				&&!themeArgs.startsWith(CommandTheme.EXCLUDE)) {
			return true;
		}
		return false;
		}
